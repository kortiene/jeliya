...
| `room.list` | `{}` | `{ rooms: [{ room_id, name, role, status, member_count, open, last_event_ts, last_event_kind }] }` — before identity creation, protocol v1 returns `{ rooms: [] }`. Afterward, `status` is this identity's roster status (`active|left|removed`) and `role` is `owner|member|agent`; unauthorized rooms are omitted. **`name` is `null`** until the genesis event syncs. Client models retain nullable `role`/`status` only for compatibility with older protocol-v1 implementations; the v0.5 daemon does not emit such a row. **`last_event_ts`** (Unix ms, nullable) is the `created_at` the newest locally-held signed event's author signed — a store projection, never the wall clock or render time — and **`last_event_kind`** (nullable) is that event's `TimelineEvent` kind. Both are **optional and read-only**: an older daemon omits them, a room with no readable event reports `null`, and `last_event_kind` is `null` for an event the kind enumeration does not name (`member.removed`) while `last_event_ts` still carries the real time. Recency reflects only what **this device has synced**, so a room whose peers are offline is truthfully stale, never presented as current. Clients must render no recency rather than fabricate one (see `docs/room-attention.md`). **Every listed room carries recency**: a room with no stored events is not listed at all, and a failed projection read is an error rather than a null, so a null `last_event_ts` on a listed row means the daemon predates the projection — never that the room is empty. |
