...
| #91 | Departed-room read-only archive: `AppRoot`'s `Route::Room` arm branches on `Standing` (`Active` → live `RoomShell`; `Left`/`Removed` → `RoomArchivePane`). The pane dispatches only the typed `room.archive` read — normatively network-free and mutation-free — and renders the permanent departure banner (signed left/removed fact, rejoin explanation), the signed timeline from the paged reply, and the historical roster reconstructed from the signed membership events (not `room.members`, which is refused for a departed caller under step-5 standing gate). Every live action is absent by construction, not present-and-disabled. A `room_still_active` race is an honest notice with Rooms as the way out. New pure host-testable module `jeliya-ui/src/room/` (`archive.rs`, `roster.rs`, `capability.rs`) and Dioxus component `components/room_archive.rs`; EN/FR l10n; integration tests in `tests/archive_91.rs`. Satisfies product behavior contract invariant 5 (the deliberate widening of the retained Room Workbench floor). Re-qualified per platform in #182. | Landed |
