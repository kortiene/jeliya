...
DirectClient Android adapter (#173), the M3 web foundation (#176), and the
M4 daemon supervisor (#170) are implemented.** Jeliya
...
| #173 | The `DirectClient` Android adapter (`crates/jeliya-client/src/direct/`): typed `jeliya-core` in-process behind one bounded serialized actor, the production replacement for `crates/jeliya-ffi/src/host.rs`. Data path contains no daemon-envelope JSON, Dart, C ABI, socket, token, or portfile. Key properties: serialized dispatch through a bounded request channel (WAL-race guard, inherited from `FfiHost`); one live owner per canonical data directory (`OwnershipRegistry`, `OwnerToken`); push loop runs for the engine's whole life so the join-bootstrap `accept_joins` window stays open; `DirectDriverTask` drives the kernel (`Core`) and routes typed calls through the shared in-process bridge (`route` → `TypedCall` → `execute_with` → `TypedReply`); `DirectEngineActor` owns the `Engine`, its push-forward task, and teardown (push loop stop → dispatch drain → `close_all_rooms` → drop); `DirectBackend` connects the kernel loop to the engine actor. `stable_principal = true` (in-process: daemon restart = client restart). Resume is `Reconciler::resume()` — no fabricated reconnect. The `direct` Cargo feature is default-off and native-only; `tokio` and `jeliya-core` appear only behind it, so the `wasm32` and `--no-default-features` graphs are unchanged. New public API: `DirectConfig`, `connect_direct`, `OwnershipError`; `jeliya-codec` gains `pub use` re-exports of `route` and `Call` for the shared bridge. | Landed |
