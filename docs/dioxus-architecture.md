...
composer/drafts/pending-send (#179), the M3 browser Files/Pipes room
...
such suite exists; of the four adapters the deterministic mock (shipped with
the seam, #167) and the browser `WsWeb` (#171) have landed. `WsWeb` lives
inside `jeliya-client` behind the default-off, wasm-only `ws-web` feature and
ships the reusable async runtime `WsNative` and `DirectClient` will reuse; it
emits connected only after validating the daemon's `hello`, obtains fresh
`/api/health` status and a fresh `/api/session` credential on every attempt,
and runs with replay disabled until the daemon-incarnation fence (#270).
Flipping `web_composition` from the mock to `WsWeb` is a later cutover, and
`WsNative` and `DirectClient` do not yet exist.
...
| #171 | The browser WebSocket/session adapter (`WsWeb`, `crates/jeliya-client/src/ws_web/`): the first real adapter for the kernel seam and the sole browser client for the v2 stack. Built on the new generic async runtime (`src/kernel/runtime.rs`) that `WsNative` (#172) and `DirectClient` (#173) reuse verbatim. Key properties: every dial obtains fresh `/api/health` status (daemon-status gate, catches protocol mismatch before the socket opens) and a fresh `/api/session` credential via the injected `SessionResolver` seam; `Connected` is emitted only after the first frame validates as a well-formed `hello` whose protocol and storage-generation match (`GateRefused` otherwise); tokens and credentials are stripped from every diagnostic via `RedactedUrl` and never stored in renderable form (`AC-4`); `stable_principal` is forced off until the daemon-incarnation fence (#270); reconnect jitter is seeded from `crypto.getRandomValues`. The **mailbox split** (§5): the `Send + Sync` backend holds the core, senders, event bus, an IO-action mailbox, and an `AtomicWaker`; the `!Send` `spawn_local` pump owns the driver's browser handles, drains the mailbox, and feeds driver events back as `Input`s — keeping `#![forbid(unsafe_code)]` clean and confining browser `!Send` concerns to the pump. Lives inside `jeliya-client` behind the default-off `ws-web` feature gated to `wasm32-unknown-unknown`; wasm-only deps (`web-sys`, `wasm-bindgen` pinned `=0.2.126` matching the reproducible-build CLI contract from #176, `js-sys`, `wasm-bindgen-futures`) are confined to a `[target.'cfg(target_arch = "wasm32")'.dependencies]` block and never enter the native dependency tree. Adds a codec client edge to `crates/jeliya-codec` (`encode_request` / `decode_client_frame` / `ClientInbound`). Verified by five `wasm_bindgen_test` real-browser scenarios (connect→Ready, push, daemon restart, protocol mismatch, stop) against a live supervised daemon; the non-required CI job `jeliya-client-ws-web` gates the wasm32 build check (AC-1: no Iroh/native deps) and the browser tests together. Flipping `jeliya-ui::compose::web_composition` from the mock to `connect_ws_web` is the production cutover — deferred. | Landed |
...
| #175 | The parameterized adapter contract suite (first slice: `crates/jeliya-client/src/contract/` + `tests/contract_suite/`, behind the default-off `contract` feature). Ten view-level contracts — lifecycle to Ready, typed-call round-trip, push fan-out (never as replies), local queue bounds, cancellation classification, exact-version gate finality, dedup ambiguity (same-body replay returns the original result; a different body under a used key is `op_id_conflict`), transport loss (`Interrupted`→`Failed` with honest settlement), restart (re-resolve, re-dial, re-establish identity, then the durable read-back both real backends promise: the daemon persists identity, the `state.json` room index, and the `rooms.db` genesis store; the direct engine's store likewise), and stop (settle, close, refuse, idempotent) — written once against the `Rig` seam and replayed against the scripted mock, `WsNative` driving a real spawned `jeliyad`, and `DirectClient` over a real in-process Engine. Every contract×rig gap carries a recorded reason and a meta-test polices the matrix. Honest per-rig differences are capabilities with recorded reasons, not waivers: a scripted oracle cannot echo inputs, and the direct adapter's actor routes `stream.*` to the engine dispatcher, which answers `subscription_unknown` (a per-connection subscription surface for direct is an open follow-up). CI runs the matrix (plus clippy under the combined feature set) with the daemon it just built — the rig resolves `JELIYAD_BIN` or the `target/debug/jeliyad` default and fails when the binary is missing, never skips. Remaining #175 scope: the wasm/web rig in a real browser, the six fresh-state real-oracle scenarios (#46/#47/#147/#79/#50/#94), and the Android target job. | Partial |
