...
| the two localization gates, which guarantee **different** things: `scripts/check-ui-i18n.mjs` is the only completeness gate, and it covers the React catalogs alone (key parity, no empty value, no French value byte-identical to its English source); `scripts/i18n-gate.mjs` covers the Flutter tree for literal externalization only, and `app/test/l10n_parity_test.dart` pins the **English** ARB's rendered output. No gate checks French ARB completeness: `l10n.yaml` writes `build/untranslated_messages.json` but nothing asserts it is empty | the only completeness gate there is, together with the catalogs it inspects | an equivalent gate enforces key parity, non-empty values, and genuine translation over the replacement catalog, and passes before either client's catalog is removed. French parity is the property most likely to regress silently, because an untranslated string still renders and still reads as working software | web maintainer |
...
**One of those properties is already unenforced.** French ARB completeness has
never been gated on the Flutter side — `flutter gen-l10n` reports untranslated
messages to `build/untranslated_messages.json`, and no check reads it. That is
a gap in the stack shipping today, not one the retirement creates, and it must
not be inherited by the replacement on the assumption that parity was ever
mechanically held on both clients. Owner: web maintainer.
