...
- [Dioxus clean-slate architecture](dioxus-architecture.md) - Decision record for the clean-slate typed Rust client stack on Dioxus system-WebView rendering, its protocol and storage generation, the single embedded artifact, and the retirement of React, Flutter, the Dart protocol, and the C ABI. Decided; the M1 typed-API slices and the M2 entry (client seam, #167) are implemented, the M1 remainders (#165/#166/#233) stay open, and the full program is in progress.
