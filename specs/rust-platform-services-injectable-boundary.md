...
The files capability never traffics in raw strings. Five distinct types carry the distinct object kinds a file surface touches, and the one *name* type is validated rather than merely promised: **`FileName`** is constructed only through `FileName::parse(...) -> Result<Self, InvalidFileName>`, which fails closed on an empty name, on `.`/`..`, on any `/` or `\` separator, and on control characters — the same discipline `SafeExternalUrl::parse` uses for schemes. A peer-supplied name therefore cannot carry *portable* path syntax into a native sink's artifact naming, and the error carries no payload (K1). What the type guarantees is path syntax, not a platform-complete naming policy: a POSIX sink may join a `FileName` under a directory it owns without re-checking traversal, while a **Windows** sink must first reject or rewrite `:` (`C:name` is a drive-qualified path, and `PathBuf::push` replaces the base when the pushed path has a prefix but no root) and trim trailing dots and spaces. Those two, plus Unicode normalization, reserved device names, and length caps, are deliberately the sink's — `10:30 notes.txt` is an ordinary file name on the committed targets, so rejecting it in the shared type would break them to close a hazard on one whose scope is undecided.
...
fn release_staged(&self, blob: &ShareableBlob)
    -> BoxFuture<'_, Result<(), CapabilityError>>;
fn discard_source(&self, src: &PickedSource)
    -> BoxFuture<'_, Result<(), CapabilityError>>;
fn release_artifact(&self, artifact: &FetchedArtifact)
    -> BoxFuture<'_, Result<(), CapabilityError>>;
fn discard_export_target(&self, target: &ExportTarget)
    -> BoxFuture<'_, Result<(), CapabilityError>>;
...
- **Abandoned handles are releasable.** The same custody rule applies before staging: a successful pick leaves the real `File`/path/`content://` grant in the service's private table while the component holds only a token, so a pick the user abandons would otherwise be retained for the service's lifetime. `discard_source`, `discard_export_target`, and `release_artifact` are the explicit release paths, one per handle that names service-held bytes or a grant. Consumption is the other half and it is unconditional — but it is a fact about the *service's private entry*, never about the caller's handle: every method **borrows**. `stage_for_share` drops its source entry on every outcome, `export_sink` consumes its target, and a successful share consumes the `FetchedArtifact`; in each case the caller keeps the handle, which then fails closed on reuse. Borrowing is what makes the cross-service case safe: a handle from service A passed to service B fails closed in B while remaining discardable on A, whereas consuming it would strand A's grant with nothing left to name it. It is also what lets a failed cleanup be retried, since the service deliberately keeps the bytes in that case. Only an abandoned handle needs an explicit release — and "abandoned" includes a handle the caller never received: a picker that binds its result and is then cancelled or dropped reaps the grant itself, because the caller has no handle to discard. A share sheet **holds** its attachment's bytes for its own lifetime rather than re-reading the registry at settlement, so releasing a blob underneath an open sheet cannot turn that share into a lie. Cleanup can itself fail (a locked temporary file, a permission change); a failed release leaves the entry recoverable so the caller can retry, which is why every cleanup **borrows** its handle: consuming it would make the retained bytes unreachable by the very call that is supposed to free them, and that path is scriptable like every other. A `Drop` guard cannot substitute — the handles are `Clone`, so no copy can know it is the last.
- **Cancellation reaches the transfer, not just its opening.** A sink carries the `CancelToken` it was opened under: a token fired mid-copy stops further writes and prevents `commit` from publishing, so a cancelled export can never become a finished artifact (§K2) and an adapter that keeps advancing CREDIT past a fired token fails against the fake rather than passing.
- **The reap is explicit.** "Delete after share" needs a signal the service cannot derive: reaching EOF in a `StagedBlobReader` means the bytes were *read*, not that the daemon accepted the operation (a retry re-reads them), and dropping the opaque `ShareableBlob` is invisible to the service. The UI drives `file.share`, so the UI calls `release_staged(blob)` once the operation has settled — success or failure — and the service deletes its staging copy. What makes a successful release final is the **registry removal**, not handle ownership: the handle is borrowed, so a deletion that itself fails (a locked temporary, a permission change) leaves the caller able to retry rather than holding nothing. Releasing a blob this service did not stage, or one already released, fails closed with `Unreadable`, and a reader already open over the bytes keeps working (an open descriptor outliving an unlink) — and counts as retained for as long as it does. This is the outbound mirror of the inbound rule that a successful share consumes a `FetchedArtifact`.
...
- **`Share::share(&ShareContent, &CancelToken) -> BoxFuture<Result<(), CapabilityError>>`** — the OS share sheet (invite tickets via `SharePlus`; Android file share). Asynchronous and cancellable, not a plain `Result`: the sheet stays open until the user settles it, so the operation is pending until then and a `CancelToken` must be able to withdraw it. Dismissal, explicit cancellation, and refusal stay distinct outcomes. `ShareContent` carries text and/or a `ShareableBlob`/`FetchedArtifact` — in both cases a handle to bytes the producing service already custodies, never a bare id or path — plus an optional anchor rect for iPad popover presentation. Dismissing the share sheet is `Cancelled`, not `Ok`. The content is taken **by reference**: a denied, dismissed, or dropped sheet must leave the caller holding its attachment handle, or the bytes the service still owns become unreachable — a `ShareableBlob` is never consumed by sharing, and a `FetchedArtifact` only by a share that completes.
...
`Navigation` exposes `route() -> Route`, `navigate(Route)`, and consumes `BackRequested`/`NavigationRequested` from the lifecycle stream. Route ids get one deliberate divergence from `encodeURIComponent`: a segment that is entirely dots would be a URL **dot segment**, and a browser would normalize `/rooms/../activity` to `/activity` before the router ever saw it, silently losing the room. No percent-encoding avoids this — the URL Standard treats `%2e` exactly like `.` — so the `.`\*`!`\* family shifts by one, reversibly. The same escape covers the other unrepresentable segment, the **empty** id: `""` spells `!`, which is why `to_path` stays the exact inverse of `parse` (an empty id previously rendered `/rooms//activity`, which does not parse, and an empty selected file id rendered a trailing slash that parsed back as *no selection*). Every id with a non-dot character, including every `blake3:…`, keeps its byte-identical spelling. `Route` is a typed, parsed route (not a raw string), so the router and the platform agree on one state machine — the product contract's "the URL *is* the navigation state; no second state machine may disagree with it". On the browser the impl is the history/hash route; on desktop/Android an in-memory router. The last-open-room restore (D6 preference) is applied **once per launch, only from the bare root**, and always loses to an explicit route — a rule the navigation + preferences capabilities express together, not a component `cfg`.
...
- **Scriptable outcomes.** Any capability method can be scripted to return `Unavailable` / `Denied` / `Cancelled` / a typed `Failed`, so the denied/unavailable/cancelled paths the Verification section demands are exercisable without a device. This includes the two failure paths a sink has beyond "it never opened": **mid-transfer** (`FileSink::write` / `ShareSink::write` — a disk that fills or a SAF grant revoked partway through, where the caller must stop advancing CREDIT, abort the stream, and drop the uncommitted artifact) **source-side** (`StagedBlobReader::next_chunk` — the staged file becoming unreadable after the reader was opened, where the uploader must stop sending DATA and settle without reaping bytes a retry needs), and **finalization** (`FileSink::commit` / `ShareSink::commit` — a flush error, a document that will not close, a download that never publishes, where every byte was accepted and the caller must still not report success). A fake that always accepts every chunk and always commits cannot exercise either. The same applies to cleanup (a locked temporary file, a permission change — a failed release leaves the entry recoverable so the caller can retry) to the two failure moments a staged reader has (opening, and pulling), and to **window actions** on a platform that has windows: a compositor refusing a raise or an OS vetoing a close is an action outcome, distinct from the structural `Unavailable` a browser or Android shape reports.
- **Three shapes (`fake::shapes`)** so shared components compile and behave against each target's structural facts:
  - **Browser fake** — `Preferences::durability() == SessionScoped`; `PrivateDirectory`/`WindowActions` `Unavailable`; `PickedSource` kind `BrowserBlob`; staging keeps the picked blob in **client-side custody** and serves it through `read_staged` for the v2 `file.share` byte stream (bytes recorded, no local staging dir). It deliberately does *not* model the retired v1 server-side upload edge, which protocol v2 removed — recreating that edge is the mistake this fixture exists to make impossible.
