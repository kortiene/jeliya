...
- **Abandoned handles are releasable.** The same custody rule applies before staging: a successful pick leaves the real `File`/path/`content://` grant in the service's private table while the component holds only a token, so a pick the user abandons would otherwise be retained for the service's lifetime. `discard_source` and `discard_export_target` are the explicit release paths; `stage_for_share` and `export_sink` consume their handles, so only an abandoned one needs discarding. A `Drop` guard cannot substitute — the handles are `Clone`, so no copy can know it is the last.
