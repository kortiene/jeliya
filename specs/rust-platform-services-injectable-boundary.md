...
- **Scriptable outcomes.** Any capability method can be scripted to return `Unavailable` / `Denied` / `Cancelled` / a typed `Failed`, so the denied/unavailable/cancelled paths the Verification section demands are exercisable without a device. This includes the two failure paths a sink has beyond "it never opened": **mid-transfer** (`FileSink::write` / `ShareSink::write` — a disk that fills or a SAF grant revoked partway through, where the caller must stop advancing CREDIT, abort the stream, and drop the uncommitted artifact) **source-side** (`StagedBlobReader::next_chunk` — the staged file becoming unreadable after the reader was opened, where the uploader must stop sending DATA and settle without reaping bytes a retry needs), and **finalization** (`FileSink::commit` / `ShareSink::commit` — a flush error, a document that will not close, a download that never publishes, where every byte was accepted and the caller must still not report success). A fake that always accepts every chunk and always commits cannot exercise either. The same applies to cleanup (a locked temporary file, a permission change — a failed release leaves the entry recoverable so the caller can retry) and to **window actions** on a platform that has windows: a compositor refusing a raise or an OS vetoing a close is an action outcome, distinct from the structural `Unavailable` a browser or Android shape reports.
