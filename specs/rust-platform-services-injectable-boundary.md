...
- **Abandoned handles are releasable.** The same custody rule applies before staging: a successful pick leaves the real `File`/path/`content://` grant in the service's private table while the component holds only a token, so a pick the user abandons would otherwise be retained for the service's lifetime. `discard_source`, `discard_export_target`, and `release_artifact` are the explicit release paths, one per handle that names service-held bytes or a grant. Consumption is the other half and it is unconditional: `stage_for_share` drops its source entry on *every* outcome (it took the handle by value, so no caller can discard it afterwards), `export_sink` consumes its target, and a successful share consumes the `FetchedArtifact`. Only an abandoned handle needs an explicit release — and "abandoned" includes a handle the caller never received: a picker that binds its result and is then cancelled or dropped reaps the grant itself, because the caller has no handle to discard. A share sheet **holds** its attachment's bytes for its own lifetime rather than re-reading the registry at settlement, so releasing a blob underneath an open sheet cannot turn that share into a lie. Cleanup can itself fail (a locked temporary file, a permission change); a failed release leaves the entry recoverable so the caller can retry, which is why every cleanup **borrows** its handle: consuming it would make the retained bytes unreachable by the very call that is supposed to free them, and that path is scriptable like every other. A `Drop` guard cannot substitute — the handles are `Clone`, so no copy can know it is the last.
