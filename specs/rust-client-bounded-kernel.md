...
- **Status of this document:** implemented. The request/reply kernel ships as `crates/jeliya-client/src/kernel/` (modules: `core`, `admission`, `ids`, `inflight`, `replay`, `backoff`, `timing`, `transport`, `diag`), with fault coverage in `tests/kernel_fault.rs` and the deterministic in-memory driver behind `feature = "test-transport"`. The **stream lifecycle hooks §K16 describes shipped with #269** (module `kernel/streaming`, seam extension in `kernel/transport`, happy-path lifecycle in `tests/kernel_stream.rs`); its finished design record is `specs/rust-client-kernel-stream-lifecycle.md`. This document is the authoritative design record for the request/reply kernel; where it and the code disagree the code is right and this document has a bug.
...
> **Note on replay eligibility (amended in review; resolves §14 Q3 as ADOPTED).** Replay requires ALL of: `mutating`, a caller `op_id`, membership in the protocol's 13-operation **`op_id`-deduplicated set** (the only operations whose replay returns the *original* result), and the driver's stable-principal certification below. The earlier `mutating && op_id` heuristic was overturned in review: outside the dedup set a replay returns the SECOND invocation's view — an ensure's `created: false` when the lost original created, or `daemon.stop`'s `shutdown_in_progress`, whose typed-error classification would falsely claim `DefinitelyNot` for a stop that executed. Those are wrong answers, not duplicate ones, so nothing outside the set auto-replays; it settles `Disconnected { Unknown }` honestly instead. Replay additionally requires the driver's **stable-principal certification** (`KernelConfig::stable_principal`, default **false**): the ledger is keyed `(principal, op_id)`, so an adapter that omits `client_id` receives a fresh ephemeral principal per connection, where a replay would re-execute a lost-reply mutation. The certification covers the **complete dedup scope**: the principal AND the daemon incarnation — the ledger is deliberately in-memory, so a daemon restart empties it while the storage-generation gate still passes; `DirectClient` can certify (in-process), a socket adapter cannot until the protocol's `hello` carries a daemon incarnation identity (tracked as a protocol follow-up). Without the certification every call is `Never` and a disconnect settles honestly as `Disconnected { Unknown }`.
...
- **Connection-attempt identity.** Every `Action::Dial` carries a fresh `token`; every outcome of that attempt — `Connected`, `DialFailed`, `GateRefused` — echoes it, and the core accepts only the outstanding dial's token. A **live-connection loss** (`Input::Interrupted`) is instead tagged with the connection **generation** and accepted only while `Ready`. Together these make every connection input attributable to exactly one attempt or connection: duplicate closes, retired-dial stragglers, and delayed callbacks from replaced connections all drop, and a spurious loss can never burn a retry from the budget or block the real completion. The driver obligation is echo discipline: one outcome per dial attempt, tagged with that attempt's token; losses tagged with the generation the transport connected under.
...
> **Status:** the stream lifecycle hooks this section describes **shipped with #269**; its finished design record is `specs/rust-client-kernel-stream-lifecycle.md`. `call_stream::<FileShare>`/`::<FileRead>` now drive a full `OPEN/DATA/CREDIT/END/ABORT` lifecycle through `crates/jeliya-client/src/kernel/streaming.rs` (the byte-free sub-core), with the per-stream absolute deadline, stall timer, credit-bounded outbound bytes, the transport seam's binary-record concept, and the `file.share`/`file.read` replay gate (§S8). This paragraph is retained as the original scope statement.

`file.share`/`file.read` are duplex byte streams (protocol §Byte-stream framing; owned by #233/#242/#243 daemon-side and #167's `StreamCall` surface client-side). #168 shipped the **request/reply kernel** in full; the **stream lifecycle hooks** this section describes — per-stream absolute deadline (connect allowance + floor-throughput term), stall timer, credit-bounded outbound bytes, and `OPEN/DATA/CREDIT/END/ABORT` state as a thin layer over the same core driving the existing `StreamCall` — were **owned by #269** (they also need the transport seam to grow a binary-frame concept, which is a slice of its own). Full byte-stream execution and its fixtures land with #233's executor; #269 wires the surface to real deadlines/credit and **does not** re-implement the framing rules, doc-pointed at #233/#242/#243.
...
3. **Replay eligibility source of truth.** *Resolved in review: the exact table is adopted* — `replay.rs` gates on the protocol's 13-operation `op_id`-deduplicated set (mirroring `docs/protocol-v2.md` §Idempotency and the engine's `is_dedup_op`), because the heuristic did not merely under-replay: outside the set a replay returns the second invocation's view, a wrong answer (see the §K5 note). Exporting the set from `jeliya_api::Operation` as a `const REPLAY` so client and daemon share one definition remains a worthwhile follow-up for the adapter slice.
