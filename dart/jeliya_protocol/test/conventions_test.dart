...
''');
  ...
  // Mirror of `shouldSeedFromLiveEvent` in ui/src/lib/lastSeen.test.ts, so both
  // clients establish the unread baseline by the same rule (issue #154).
  group('shouldSeedFromLiveEvent (baseline for a no-recency daemon)', () {
    test('seeds when the listed row carries no recency', () {
      // The daemon lists no room without events, so a null here is not "empty
      // room"; it is "this daemon predates the recency projection".
      expect(shouldSeedFromLiveEvent(_recencyRoom(_room, null, null)), isTrue);
    });

    test('does NOT seed when the row has recency, so it cannot mask activity', () {
      expect(shouldSeedFromLiveEvent(_recencyRoom(_room, 100, 'message')), isFalse);
      expect(shouldSeedFromLiveEvent(_recencyRoom(_room, 0, 'message')), isFalse);
    });

    test('does NOT seed for a room that is not listed yet', () {
      expect(shouldSeedFromLiveEvent(null), isFalse);
    });

    test('an event observed BEFORE the first room.list still becomes the baseline', () {
      // The reviewer's ordering case (#154): a room can push before room.list
      // populates. Because the baseline is resolved where BOTH the rows and the
      // live map are visible — _seedUnread, re-run on every push — that event
      // seeds the mark rather than falling through the gap.
      int? seen;
      const early = 500;

      // Push first: no rows yet, so nothing can be seeded.
      const List<RoomSummary> noRowsYet = [];
      for (final row in noRowsYet) {
        if (shouldSeedFromLiveEvent(row)) seen ??= early;
      }
      expect(seen, isNull);

      // The row then lands with no recency, and the earlier event seeds it.
      final row = _recencyRoom(_room, null, null);
      if (shouldSeedFromLiveEvent(row)) seen ??= early;
      expect(seen, early, reason: 'the pre-list event was not lost');
      expect(roomUnread(_recencyRoom(_room, early, 'message'), seen), isFalse);

      // The NEXT event flags, rather than becoming a second baseline.
      if (shouldSeedFromLiveEvent(row)) seen ??= 900;
      expect(seen, early);
      expect(roomUnread(_recencyRoom(_room, 900, 'message'), seen), isTrue);
    });

    test('absorbs the first event as a baseline, then flags every later one', () {
      // Without a baseline such a daemon could never raise a dot at all; with
      // one, only the first observed event is absorbed.
      int? seen;
      final row = _recencyRoom(_room, null, null);

      if (shouldSeedFromLiveEvent(row)) seen ??= 500;
      expect(roomUnread(_recencyRoom(_room, 500, 'message'), seen), isFalse);
